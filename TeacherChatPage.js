...
const TeacherChatPage = ({ route }) => {
  ...
  const fetchMessageHistory = async () => {
    try {
      const class_name = String(teacher.class);
    ...
    try {
      await fetch('https://b7b1-2405-201-c007-3923-1514-8e5d-e869-8024.ngrok-free.app/send-message/', {
        ...
        body: JSON.stringify({
          ...
          message,
          class_name: String(teacher.class),
          role: 'Teacher',
          chat_type: 'group',
        }),
      });
  ...
  const isCurrentUser = (sender) => {
    return sender === teacher.username;
  };

  return (
    <View style={styles.container}>
      <Text style={styles.header}>Teacher Chat Page</Text>
      <FlatList
        data={messages}
        renderItem={({ item }) => (
          <View style={[
